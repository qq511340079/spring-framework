...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//初始化ApplicationEvent广播器，用来回调实现了ApplicationListener接口的bean
				initApplicationEventMulticaster();

				// Initialize other special beans in specific context subclasses.
                // 子类可以实现该方法用来执行其它的初始化逻辑
				onRefresh();

				// Check for listener beans and register them.
				// 将ApplicationListener注册到上面初始化的ApplicationEventMulticaster
                registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
                // 实例化所有剩下的单例bean(除了懒加载的)
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        //添加ApplicationContextAwareProcessor(BeanPostProcessor)，负责调用bean实现的ApplicationContextAware等接口的回调方法
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		//忽略这些Aware接口，因为在ApplicationContextAwareProcessor会调用接口相关的回调方法
        ...
        // 注册可解析的依赖Class对应的实例，比如对象A中依赖了BeanFactory类，spring在进行依赖注入的时候就会取这里设置的beanFactory注入
        ...
        //如果beanFactory中包含loadTimeWeaver，则添加LoadTimeWeaverAwareProcessor，负责调用实现了LoadTimeWeaverAware接口的bean回调方法
	...
	protected void registerListeners() {
        ...
        // 寻找BeanFactory中ApplicationListener类型的bean，只找常规的bean，不初始化FactoryBean
		String[] listenerBeanNames = getBeanNamesForType(ApplicationListener.class, true, false);
		// 遍历listenerBeanNames，将listenerBeanName添加到事件广播器
		for (String listenerBeanName : listenerBeanNames) {
			getApplicationEventMulticaster().addApplicationListenerBean(listenerBeanName);
		}

		// Publish early application events now that we finally have a multicaster...
        // 已经将ApplicationListener注册到了事件广播器了，现在可以广播需要提早广播的ApplicationEvent了
